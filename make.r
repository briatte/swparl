...
for(ii in unique(na.omit(b$legislature))) {
  ...
  i = s[ V(nn)$name, "party" ]
  i[ i %in% c("IND") ] = NA # ignoring: many unaffiliateds
  ...
  if(plot) {
    ...
    ggsave(paste0("plots/net_ch", ii, ".pdf"), 
           g + theme(legend.key = element_blank()),
           width = 10, height = 9)
    ggsave(paste0("plots/net_ch", ii, ".jpg"),
  ...
  if(gexf) {
    ...
    meta = list(creator = "rgexf",
                ...
                keywords = "parliament, switzerland")
    
    node.att = data.frame(url = gsub("/f/suche/pages/biografie.aspx\\?biografie_id=", "", n %v% "url"), # simplify URL
                          party = n %v% "partyname",
                          bills = n %v% "n_bills",
                          distance = round(n %v% "distance", 1),
                          photo = n %v% "photo",
                          # extra
                          canton = n %v% "constituency",
